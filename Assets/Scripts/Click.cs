...
public class Click : MonoBehaviour {
    ...
    private void Update () {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                if (hit.collider.gameObject.tag == "obj") // 오브젝트 클릭했으면
                {
                    clickedObj = hit.collider.gameObject;
                    GetBtn.SetActive(true); // 클릭 시 채집 버튼 뜨도록
                    GetBtn.transform.position = new Vector2(clickedObj.transform.position.x + 1.3f, clickedObj.transform.position.y + 0.8f);
                }
            }
